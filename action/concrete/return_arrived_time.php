...
if($_POST['login_id']) {
	...
	if(isSet($idArray[0]['last_name'])) {
		...
		foreach($return_arrived_time as $key){
			if($key=='return_arrived_time'){
		...
		if($ip==$ban_ip){
			...
			try{
				...
				$errors = $form_validate->validate_form_return_arrived_time($datas);
				...
				if($errors){
					...
					//データベースへ投入
					concrete::return_arrived_time($datas);
					echo 'SUCCESS';
