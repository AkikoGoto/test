...
if($_POST['login_id']) {
	...
	if(isSet($idArray[0]['last_name'])) {
		...
		foreach($return_leaving_time as $key){
			if($key=='return_leaving_time'){	
		...
		if($ip==$ban_ip){
			...
			try{
				...
				$errors = $form_validate->validate_form_return_leaving_time($datas);
				...
				if($errors){
					...
					echo 'INVALID_ACCESS';
						...
						//データベースへ投入
						concrete::return_leaving_time($datas);
						echo 'SUCCESS';
