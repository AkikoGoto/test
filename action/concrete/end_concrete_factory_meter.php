...
if($_POST['login_id']) {
	...
	if(isSet($idArray[0]['last_name'])) {
		...
		foreach($end_concrete_factory_meter as $key){
		...
		if($ip==$ban_ip){
			...
			try{
				...
				$errors = $form_validate->validate_form_end_factory_meter($datas);
				...
				if($errors){
					...
					//データベースへ投入
					$concrete_attendance_id = concrete::end_factory_meters($datas);
