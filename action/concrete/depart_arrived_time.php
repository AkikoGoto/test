...
if($_POST['login_id']) {
	...
	if(isSet($idArray[0]['last_name'])) {
		...
		foreach($depart_arrived_time as $key){
			if($key=='depart_arrived_time'){	
		...
		if($ip==$ban_ip){
			...
			try{
				...
				$errors = $form_validate->validate_form_depart_arrived_time($datas);
				...
				if($errors){
					...
					echo 'INVALID_ACCESS';
						...
						//データベースへ投入
						concrete::depart_arrived_time($datas);
						echo 'SUCCESS';
