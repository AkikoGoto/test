...
if($_POST['login_id']) {
	...
	if(isSet($idArray[0]['last_name'])) {
		...
		foreach($concrete_driver_status_end_time as $key){
			if($key=='end_time'){
		...
		if($ip==$ban_ip){
			...
			try{
				...
				$errors = $form_validate->validate_form_concrete_driver_status_end_time($datas);
				...
				if($errors){
					...
					//データベースへ投入
					concrete::concrete_driver_status_end_time($datas);
